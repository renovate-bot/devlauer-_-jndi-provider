...
public class ENCFactory implements ObjectFactory {
	...
	// Attributes ----------------------------------------------------
	private static final WeakHashMap<ClassLoader, Context> classloaderKeyedEncs = new WeakHashMap<>();
	private static final ThreadLocalStack<Object> encIdStack = new ThreadLocalStack<>();
	private static final ConcurrentHashMap<Object, Context> encById = new ConcurrentHashMap<>();
	...
	public Object getObjectInstance(Object obj, Name name, Context nameCtx, Hashtable<?, ?> environment) throws Exception {
		...
		if (currentId != null) {
			Context compCtx = encById.get(currentId); //NOSONAR - better Exception handling
			if (compCtx == null) {
		...
		synchronized (classloaderKeyedEncs) {
			Context compCtx = classloaderKeyedEncs.get(ctxClassLoader); //NOSONAR - better exception handling
