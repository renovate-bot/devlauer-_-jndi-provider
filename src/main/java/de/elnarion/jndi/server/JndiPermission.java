 ...
 * pathname consisting of the special token "&lt;&lt;ALL BINDINGS&gt;&gt;"
...
public final class JndiPermission extends Permission implements Serializable {
	...
	/**
	 * All actions (bind, rebind, unbind, lookup, list, listBindings,
	 * createSubcontext)
	 */
	public final static int ALL = LOOKUP | REBIND | BIND | UNBIND | LIST | LIST_BINDINGS | CREATE_SUBCONTEXT;
	...
	private static final String[] ALL_ACTIONS = { BIND_ACTION, REBIND_ACTION, UNBIND_ACTION, LOOKUP_ACTION, LIST_ACTION,
			LIST_BINDINGS_ACTION, CREATE_SUBCONTEXT_ACTION };
	...
	/**
	 * initialize a JndiPermission object. Common to all constructors. Also called
	 ...
	 * comma-separated list of the desired actions granted on the file or directory.
	 * Possible actions are "bind", "rebind", "unbind", "lookup", "list",
	 * "listBindings", and "createSubcontext".
	 * 
	 * <p>
	 * A pathname that ends in "/*" (where "/" is the file separator character,
	 * <code>'/'</code>) indicates all the files and directories contained in that
	 * directory. A pathname that ends with "/-" indicates (recursively) all files
	 ...
	 * directory, while a pathname consisting of a single "-" indicates all the
	 * files in the current directory and (recursively) all files and subdirectories
	 ...
	 * 
	 * @throws IllegalArgumentException If actions is <code>null</code>, empty or
	 *                                  contains an action other than the specified
	 *                                  possible actions.
	 */
	...
	/**
	 * Creates a new JndiPermission object using an action mask. More efficient than
	 * the JndiPermission(String, String) constructor. Can be used from within code
	 * that needs to create a JndiPermission object to pass into the
	 ...
	 * <p>
	 * <li><i>p</i>'s pathname is implied by this object's pathname. For example,
	 ...
	 *         <code>null</code> and is implied by this object, <code>false</code>
	 *         otherwise.
	 */
	...
	/**
	 * Checks if the Permission's actions are a proper subset of the this object's
	 * actions. Returns the effective mask iff the this JndiPermission's path also
	 * implies that JndiPermission's path.
	 * 
	...
	/**
	 * Checks two JndiPermission objects for equality. Checks that <i>obj</i> is a
	 * JndiPermission, and has the same pathname and actions as this object.
	 * <P>
	 * 
	 ...
	 * present actions in the following order: bind, rebind, unbind, lookup, list,
	 * listBindings, createSubcontext
	 * 
	 ...
	 * method always returns present actions in the following order: bind, rebind,
	 * unbind, lookup, list, listBindings, createSubcontext. For example, if this
	 * JndiPermission object allows both unbind and bind actions, a call to
	 * <code>getActions</code> will return the string "bind,unbind".
	 * 
	 * @return the canonical string representation of the actions.
	 */
	 ...
	 * 
	 * <p>
	 * and you are calling the <code>implies</code> method with the JndiPermission:
	 * 
	 * <pre>
	 ...
	 * "/tmp/-" and "/tmp/scratch/foo" permissions, so the effective permission is
	 * "bind,unbind", and <code>implies</code> returns true. The "implies" semantics
	 * for JndiPermissions are handled properly by the PermissionCollection object
...
final class JndiPermissionCollection extends PermissionCollection implements Serializable {
	 ...
	 *                                     JndiPermission
	 * 
	 * @exception SecurityException        - if this JndiPermissionCollection object
